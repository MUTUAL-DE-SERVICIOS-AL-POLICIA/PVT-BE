...
class UpdateRetirementFundRegulations2024 extends Seeder
{
    public function run()
    {
        ...
        DB::table('wf_states')->insert([
            ['module_id' => 3, 'role_id' => 104, 'name' => 'Área de Liquidación Fondo de Retiro', 'first_shortened' => 'Liquidación', 'sequence_number' => 7, 'created_at' => Carbon::now(), 'updated_at' => Carbon::now()]
        ]);

        DB::table('wf_sequences')->where('wf_state_current_id', 24)->update(['wf_state_next_id' => 63, 'updated_at' => Carbon::now()]);
        DB::table('wf_sequences')->where('wf_state_next_id', 47)->update(['wf_state_current_id' => 63, 'updated_at' => Carbon::now()]);       
