...
class QuotaAidCertificationController extends Controller
{
    ...
    public function printLegalDictum($id) {
        ...
        else 
        {
            ...
            $body_file .= "tiene expediente del "."POLICIA(viuda fallecida) / REFERIDO"." titular fallecido.";
        }        
        ///---ENDIFLE--////
        ...
        $body_accounts = "Que, mediante Certificación de Aportes N° ".$accounts->code." del Área de Cuentas Individuales de la Unidad de Otorgación del Fondo de Retiro Policial Solidario, Cuota y Auxilio Mortuorio, de fecha ". Util::getStringDate($accounts->date) .", se verificó los últimos "."12"." aportes antes del fallecimiento del titular / VIUDA.";
        ...
        if($quota_aid->procedure_modality_id != 14) {
            $payment .=" de ".($beneficiaries_count > 1?"los beneficiarios DE LA VIUDA ":($applicant->gender?"el beneficiario ":"la beneficiaria ")).($affiliate->gender=='M'?"del ":"de la ").$affiliate->fullNameWithDegree()." con C.I. N° ".$affiliate->identity_card." ".($affiliate->city_identity_card->first_shortened??'Sin extencion')."., en el monto de <strong>".Util::formatMoneyWithLiteral($quota_aid->total)."</strong> de la siguiente manera: <br><br>";
        } else 
        {
            $payment .= " de: ";
        }
