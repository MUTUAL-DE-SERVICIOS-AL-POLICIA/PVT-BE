...
class EcoComImportExportController extends Controller
{
    ...
    { DB::beginTransaction();
        ...
        $pago_futuro_id = 31;
        $contribution_discontinued= 41;
        try{
          $affiliate_has_not_contributions = DB::table('observables')->select('observables.observable_id')->join('affiliates','observables.observable_id','affiliates.id')->join('economic_complements','affiliates.id','economic_complements.affiliate_id')->where('observable_type', 'affiliates')->where('observation_type_id', $contribution_discontinued)->whereNull('observables.deleted_at')->whereNull('economic_complements.deleted_at')->where('economic_complements.eco_com_procedure_id','=',$current_procedures)->distinct()->get();
          $observation_disc_con = ObservationType::find($contribution_discontinued);
          ...
          $eco_com_disc_con = $eco_com_all->where('affiliate_id', $affiliate_discontinued->observable_id)->first();
              if (!$eco_com_disc_con->hasObservationType($contribution_discontinued)) {
