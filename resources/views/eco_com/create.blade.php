    ...
    <div class="row">
        <div class="col-md-12">
            <div class="ibox-content">
                ...
                @endif
                {!! Form::open(['route' => 'economic_complement_store', 'method' => 'POST', 'id'=>'eco-com-form']) !!}
                ...
                <eco-com-form inline-template>
                    <form-wizard color="#1AB394" title="" subtitle="" back-button-text="Volver" next-button-text="Siguiente" finish-button-text="Finalizar"
                        ...
                        <tab-content title="Modalidad y Requisitos" ref="uno" icon="mdi mdi-format-list-checks">
                            <eco-com-step1-requirements :eco-com-process="{{ $eco_com_process }}" :affiliate="{{ $affiliate }}"  :requirements="{{ $requirements }}" :user="{{ $user }}"
                                :cities="{{ $cities }}"  :show-requirements-error="showRequirementsError" inline-template>
                                @include('eco_com.step1_requirements')
                            </eco-com-step1-requirements>
                        </tab-content>
                        <tab-content title="Datos del Solicitante" ref="dos" icon="mdi mdi-account-edit">
                            <eco-com-step2-beneficiary :cities="{{ $cities }}" :affiliate="{{ $affiliate }}" :eco-com-beneficiary="{{ $eco_com_beneficiary }}"
                                inline-template>
                                @include('eco_com.step2_beneficiary')
                            </eco-com-step2-beneficiary>
                        </tab-content>
                        <tab-content title="Registro de rentas" icon="mdi mdi-account-multiple-plus">
                            <eco-com-step3-rents :pension-entity-id="{{ $affiliate->pension_entity_id ?? 0 }}" :eco-com="{{ $economic_complement }}" inline-template>
