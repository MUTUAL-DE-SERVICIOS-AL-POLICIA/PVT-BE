<template>
    <div>
        <quota-aid-beneficiary :beneficiary="quotaAid.applicant"
...
import QuotaAidBeneficiary from "./Beneficiary.vue";
import { mapGetters } from "vuex";
export default {
	props: ["items", "cities", "kinships"],
	data() {
		return {
		beneficiaries: []
	...
	methods: {
		...
		if (this.beneficiaries.length > 0) {
			let last_beneficiary = this.beneficiaries[
		...
		setTimeout(() => {
			...
			scrollToFooterCreateBeneficiaries(
			`#footerCreateBeneficiaries${this.beneficiaries.length - 1}`
			);
		}, 100);
		},
